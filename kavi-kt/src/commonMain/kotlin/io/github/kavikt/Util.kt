...
import io.github.kavikt.CodeBlock.Companion.isPlaceholder
...
/**
 * Returns a read-only [List] backed by [this] (a live view, not a copy): callers see updates made
 * to the backing list, but cannot mutate it through the returned reference. Used to publish a
 * [TypeVariableName]'s `bounds` field while its bounds are still being filled in for recursively
 * bounded generics. (square/kotlinpoet#1737)
 */
internal fun <T> MutableList<T>.toReadOnlyList(): List<T> =
  object : AbstractList<T>() {
    override val size: Int
      get() = this@toReadOnlyList.size

    override fun get(index: Int): T = this@toReadOnlyList[index]
  }
...
internal fun characterLiteralWithoutSingleQuotes(c: Char) =
  when {
    c == '\b' -> "\\b" // \u0008: backspace (BS)
    c == '\t' -> "\\t" // \u0009: horizontal tab (HT)
    c == '\n' -> "\\n" // \u000a: linefeed (LF)
...
internal fun formatIsoControlCode(code: Int): String = "\\u${code.toHexStr().padStart(4, '0')}"

internal fun Int.toHexStr(): String = toUInt().toString(16)
 ...
 * - And use `RegExp` directly in WasmJs for matching, using it in a similar way as in JS.
 *
...
internal val String.hasAllowedCharacters
  get() = this.any { it == ALLOWED_CHARACTER }

internal val String.allCharactersAreUnderscore
  get() = this.all { it == UNDERSCORE_CHARACTER }
...
/**
 * Because of [KT-18706](https://youtrack.jetbrains.com/issue/KT-18706) bug all aliases escaped with
 * backticks are not resolved.
 *
 ...
 * - all characters that cannot be used as identifier part (e.g. space or hyphen) are replaced with
 *   `"_U<code>"` where `code` is 4-digit Unicode character code in hexadecimal form
 */
...
private fun String.escapeIfNotJavaIdentifier(): String {
  return if (
    (!first().isJavaIdentifierStart() || drop(1).any { !it.isJavaIdentifierPart() }) &&
      !alreadyEscaped()
  ) {
    "`$this`"
  } else {
...
internal fun String.escapeSegmentsIfNecessary(delimiter: Char = '.') =
  split(delimiter)
