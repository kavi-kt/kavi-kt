        ...
        is TypeName -> o
          ...
          argToTypeOrNull(o)
            ?: throw IllegalArgumentException(
              if (o is KClass<*>) {
                "Can't convert a KClass to %T without runtime reflection (unavailable on " +
                  "Kotlin/JS and Kotlin/Wasm); pass a TypeName instead, e.g. ClassName(...)."
              } else {
                "expected type but was $o"
              }
            )
      }
...
/** Formats [o] with `.` decimal separator and `_` grouping separator, like `1_000` or `1.5`. */
internal expect fun formatNumericValue(o: Number): Any?

/**
 * Converts a JVM reflection / annotation-processing type (`java.lang.reflect.Type`,
 * `javax.lang.model.type.TypeMirror`, `javax.lang.model.element.Element`) to a [TypeName], or
 * returns null if [o] is not one. Off the JVM there are no such types, so this is always null.
 */
internal expect fun argToTypeOrNull(o: Any?): TypeName?
