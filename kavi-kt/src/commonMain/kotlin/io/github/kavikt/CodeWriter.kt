...
/**
 * Sentinel value that indicates that no user-provided package has been set. Compared by reference
 * identity, so it must be a distinct instance that can never equal a real (possibly empty) package
 * name; the contents are never emitted.
 */
private val NO_PACKAGE = "\u0000NO_PACKAGE\u0000"
...
internal inline fun buildCodeString(builderAction: CodeWriter.() -> Unit): String {
  ...
  CodeWriter(stringBuilder, columnLimit = Int.MAX_VALUE).use { it.builderAction() }
  ...
  companion object {
    ...
    fun withCollectedImports(
      ...
      val importsCollector =
        CodeWriter(NullAppendable, indent, memberImports, columnLimit = Int.MAX_VALUE)
