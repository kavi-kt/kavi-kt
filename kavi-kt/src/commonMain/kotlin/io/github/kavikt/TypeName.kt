 ...
 * Enum<E>>` terminate instead of overflowing the stack. (square/kotlinpoet#1737)
 */
...
/**
 * Tracks, by reference identity, the [TypeName] pairs currently being compared.
 * (square/kotlinpoet#1737)
 */
internal class RecursiveComparison {
  // A flat list of (left, right) pairs compared by reference identity. Cycles in type-name graphs
  // are tiny (a recursively bounded variable revisits a handful of nodes), so a linear scan is
  // cheaper than the allocation an identity map would need.
  private val left = ArrayList<TypeName>()
  private val right = ArrayList<TypeName>()

  /**
   * Records ([leftNode], [rightNode]). Returns false if that exact pair is already being compared.
   */
  fun enter(leftNode: TypeName, rightNode: TypeName): Boolean {
    for (index in left.indices) {
      if (left[index] === leftNode && right[index] === rightNode) return false
    }
    left += leftNode
    right += rightNode
    return true
