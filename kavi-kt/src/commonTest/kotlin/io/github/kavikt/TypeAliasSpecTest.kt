...
class TypeAliasSpecTest {
  ...
  @Test
  fun simpleTypeAlias() {
    val typeAliasSpec = TypeAliasSpec.builder("Word", STRING).build()
  ...
  fun typeVariable() {
    ...
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", LIST.parameterizedBy(v)).addTypeVariable(v).build()
  ...
  fun publicVisibility() {
    val typeAliasSpec = TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.PUBLIC).build()
  ...
  fun internalVisibility() {
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.INTERNAL).build()
  ...
  fun privateVisibility() {
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.PRIVATE).build()
  ...
  @Test
  fun implTypeAlias() {
    val atomicReference = ClassName("java.util.concurrent.atomic", "AtomicReference")
    val typeName = atomicReference.parameterizedBy(TypeVariableName("V"))
    val typeAliasSpec =
  ...
  fun kdoc() {
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", STRING)
  ...
  fun annotations() {
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", STRING)
        .addAnnotation(
          AnnotationSpec.builder(ClassName("io.github.kavikt", "TypeAliasAnnotation"))
    ...
    assertThat(typeAliasSpec.toString())
      .isEqualTo(
        """
        |@io.github.kavikt.TypeAliasAnnotation(value = "words!")
        |public typealias Word = kotlin.String
        |"""
          .trimMargin()
      )
  ...
  fun kdocWithoutNewLine() {
    val typeAliasSpec =
      TypeAliasSpec.builder("Word", STRING)
        .addKdoc("Word is just a type alias for [String](%T).", STRING)
        .build()
  ...
  fun equalsAndHashCode() {
    val a = TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.PUBLIC).build()
    val b = TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.PUBLIC).build()
  ...
  @Test
  fun modifyModifiers() {
    val builder = TypeAliasSpec.builder("Word", STRING).addModifiers(KModifier.PRIVATE)
  ...
  fun modifyTypeVariableNames() {
    val builder = TypeAliasSpec.builder("Word", STRING).addTypeVariable(TypeVariableName("V"))
  ...
  fun modifyAnnotations() {
    ...
    builder.annotations.add(secondValue)

    assertThat(builder.build().annotations).containsExactly(secondValue)
  }

  @Test
  fun nameEscaping() {
    val typeAlias = TypeAliasSpec.builder("fun", STRING).build()
