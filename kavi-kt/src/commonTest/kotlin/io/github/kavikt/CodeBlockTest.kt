...
class CodeBlockTest {
  ...
  fun repeatedNamedArgument() {
    ...
    assertThat(block.toString())
      .isEqualTo("\"I like \" + \"tacos\" + \". Do you like \" + \"tacos\" + \"?\"")
  }
  ...
  fun namedAndNoArgFormat() {
    val map = LinkedHashMap<String, Any>()
    map["text"] = "tacos"
    val block = CodeBlock.builder().addNamed("⇥\n%text:L for\n⇤%%3.50", map).build()
    assertThat(block.toString()).isEqualTo("\n  tacos for\n%3.50")
  }

  @Test
  fun missingNamedArgument() {
    assertFailure {
        val map = LinkedHashMap<String, Any>()
  ...
  // kmp: this test was extracted: multipleNamedArguments

  // kmp: this test was extracted: namedNewline

  @Test
  fun danglingNamed() {
    val map = LinkedHashMap<String, Any>()
    map["clazz"] = INT
    assertFailure { CodeBlock.builder().addNamed("%clazz:T%", map).build() }
      .isInstanceOf<IllegalArgumentException>()
  ...
  @Test
  fun indexTooHigh() {
    assertFailure { CodeBlock.builder().add("%2T", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
  ...
  @Test
  fun indexIsZero() {
    assertFailure { CodeBlock.builder().add("%0T", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
  ...
  @Test
  fun indexIsNegative() {
    assertFailure { CodeBlock.builder().add("%-1T", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("invalid format string: '%-1T'")
  }

  @Test
  fun indexWithoutFormatType() {
    assertFailure { CodeBlock.builder().add("%1", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("dangling format characters in '%1'")
  }

  @Test
  fun indexWithoutFormatTypeNotAtStringEnd() {
    assertFailure { CodeBlock.builder().add("%1 taco", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("invalid format string: '%1 taco'")
  }

  @Test
  fun indexButNoArguments() {
    assertFailure { CodeBlock.builder().add("%1T").build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("index 1 for '%1T' not in range (received 0 arguments)")
  }

  @Test
  fun formatIndicatorAlone() {
    assertFailure { CodeBlock.builder().add("%", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("dangling format characters in '%'")
  }

  @Test
  fun formatIndicatorWithoutIndexOrFormatType() {
    assertFailure { CodeBlock.builder().add("% tacoString", String::class).build() }
      .isInstanceOf<IllegalArgumentException>()
      .hasMessage("invalid format string: '% tacoString'")
  }

  // kmp: this test was extracted: sameIndexCanBeUsedWithDifferentFormats

  @Test
  fun tooManyStatementEnters() {
    val codeBlock = CodeBlock.builder().addStatement("print(«%L»)", "1 + 1").build()
    assertFailure {
  ...
  @Test
  fun statementExitWithoutStatementEnter() {
    val codeBlock = CodeBlock.builder().addStatement("print(%L»)", "1 + 1").build()
    assertFailure {
        // We can't report this error until rendering type because code blocks might be composed.
        codeBlock.toString()
  ...
  fun nullableType() {
    ...
    val map =
      (MAP.copy(nullable = true) as ClassName)
        .parameterizedBy(STRING.copy(nullable = true), list)
        .copy(nullable = true)
    ...
    val rarr = WildcardTypeName.producerOf(STRING.copy(nullable = true))
    val rarrBlock = CodeBlock.of("%T", rarr)
    assertThat(rarrBlock.toString()).isEqualTo("out kotlin.String?")
  }

  @Test
  fun withoutPrefixMatching() {
    assertThat(CodeBlock.of("abcd %S efgh %S ijkl", "x", "y").withoutPrefix(CodeBlock.of("")))
      .isEqualTo(CodeBlock.of("abcd %S efgh %S ijkl", "x", "y"))
    assertThat(CodeBlock.of("abcd %S efgh %S ijkl", "x", "y").withoutPrefix(CodeBlock.of("ab")))
      .isEqualTo(CodeBlock.of("cd %S efgh %S ijkl", "x", "y"))
    assertThat(CodeBlock.of("abcd %S efgh %S ijkl", "x", "y").withoutPrefix(CodeBlock.of("abcd ")))
      .isEqualTo(CodeBlock.of("%S efgh %S ijkl", "x", "y"))
    assertThat(
        CodeBlock.of("abcd %S efgh %S ijkl", "x", "y").withoutPrefix(CodeBlock.of("abcd %S", "x"))
      )
  ...
  fun withoutPrefixNoArgs() {
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("")))
      .isEqualTo(CodeBlock.of("abcd %% efgh %% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("ab")))
      .isEqualTo(CodeBlock.of("cd %% efgh %% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd ")))
      .isEqualTo(CodeBlock.of("%% efgh %% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %%")))
      .isEqualTo(CodeBlock.of(" efgh %% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %% ef")))
      .isEqualTo(CodeBlock.of("gh %% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %% efgh ")))
      .isEqualTo(CodeBlock.of("%% ijkl"))
    assertThat(CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %% efgh %%")))
      .isEqualTo(CodeBlock.of(" ijkl"))
    assertThat(
        CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %% efgh %% ij"))
      )
      .isEqualTo(CodeBlock.of("kl"))
    assertThat(
        CodeBlock.of("abcd %% efgh %% ijkl").withoutPrefix(CodeBlock.of("abcd %% efgh %% ijkl"))
      )
      .isEqualTo(CodeBlock.of(""))
  }

  @Test
  fun withoutPrefixArgMismatch() {
    assertThat(
        CodeBlock.of("abcd %S efgh %S ijkl", "x", "y")
  ...
  @Test
  fun withoutPrefixFormatPartMismatch() {
    assertThat(
        CodeBlock.of("abcd %S efgh %S ijkl", "x", "y")
  ...
  @Test
  fun trimNoPlaceholders() {
    assertThat(CodeBlock.of("return null").trim()).isEqualTo(CodeBlock.of("return null"))
  }

  @Test
  fun trimPlaceholdersWithArgs() {
    assertThat(CodeBlock.of("return %S", "taco").trim())
      .isEqualTo(CodeBlock.of("return %S", "taco"))
  }

  @Test
  fun trimNoArgPlaceholderMiddle() {
    assertThat(CodeBlock.of("this.taco = %S", "taco").trim())
      .isEqualTo(CodeBlock.of("this.taco = %S", "taco"))
  }

  @Test
  fun trimNoArgPlaceholderStart() {
    assertThat(CodeBlock.of("⇥return ").trim()).isEqualTo(CodeBlock.of("return "))
  }

  @Test
  fun trimNoArgPlaceholderEnd() {
    assertThat(CodeBlock.of("return ⇥").trim()).isEqualTo(CodeBlock.of("return "))
  }

  @Test
  fun trimNoArgPlaceholdersStartEnd() {
    assertThat(CodeBlock.of("«return this»").trim()).isEqualTo(CodeBlock.of("return this"))
  }

  @Test
  fun trimMultipleNoArgPlaceholders() {
    assertThat(CodeBlock.of("«return if (x > %L) %S else %S»", 1, "a", "b").trim())
      .isEqualTo(CodeBlock.of("return if (x > %L) %S else %S", 1, "a", "b"))
  }

  @Test
  fun trimOnlyNoArgPlaceholders() {
    assertThat(CodeBlock.of("«»⇥⇤").trim()).isEqualTo(CodeBlock.of(""))
  }
  ...
  @Test
  fun beginControlFlowWithParams() {
    val controlFlow =
      CodeBlock.builder()
        ...
        .endControlFlow()
        .build()
    assertThat(controlFlow.toString())
      .isEqualTo(
        """
  ...
  @Test
  fun beginControlFlowWithParamsAndTemplateString() {
    val controlFlow =
      CodeBlock.builder()
  ...
  @Test
  fun nonWrappingControlFlow() {
    val file =
      FileSpec.builder("com.squareup.tacos", "Test")
        .addFunction(
  ...
  @Test
  fun dontEscapeBackslashesInRawStrings() {
  ...
  // kmp: this test was extracted: useUnderscoresOnLargeDecimalLiterals

  // kmp: this test was extracted: minusSignInSwedishLocale

  @Test
  fun nameFromContextParameter() {
