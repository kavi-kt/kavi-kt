...
class CrossplatformTest {
  ...
  fun crossplatform() {
    ...
    val expectSpec =
      TypeSpec.classBuilder(expectType)
        ...
        .build()
    val actualName =
      ClassName("java.util.concurrent.atomic", "AtomicReference").parameterizedBy(expectTypeParam)
    val actualSpec =
  ...
  fun expectWithSecondaryConstructors() {
    val expectSpec =
      TypeSpec.classBuilder("IoException")
        ...
        .addFunction(FunSpec.constructorBuilder().addParameter("message", STRING).build())
        .build()
  ...
  fun topLevelProperties() {
    val fileSpec =
      FileSpec.builder("", "Test")
        .addProperty(PropertySpec.builder("bar", STRING, KModifier.EXPECT).build())
        .addProperty(
          PropertySpec.builder("bar", STRING, KModifier.ACTUAL)
  ...
  fun topLevelFunctions() {
    val fileSpec =
      FileSpec.builder("", "Test")
        .addFunction(FunSpec.builder("f1").addModifiers(KModifier.EXPECT).returns(INT).build())
        .addFunction(
          FunSpec.builder("f1")
            .addModifiers(KModifier.ACTUAL)
            .returns(INT)
  ...
  fun expectPropertyInitializerForbidden() {
    assertFailure {
        TypeSpec.classBuilder("AtomicRef")
          .addModifiers(KModifier.EXPECT)
          .addProperty(PropertySpec.builder("a", BOOLEAN).initializer("true").build())
      }
      .isInstanceOf<IllegalArgumentException>()
  ...
  fun expectPropertyGetterForbidden() {
    assertFailure {
        TypeSpec.classBuilder("AtomicRef")
          .addModifiers(KModifier.EXPECT)
          .addProperty(
            PropertySpec.builder("a", BOOLEAN)
