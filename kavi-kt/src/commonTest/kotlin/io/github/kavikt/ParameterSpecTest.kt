...
class ParameterSpecTest {
  ...
  fun equalsAndHashCode() {
    var a = ParameterSpec.builder("foo", INT).build()
    var b = ParameterSpec.builder("foo", INT).build()
    assertThat(a == b).isTrue()
    assertThat(a.hashCode()).isEqualTo(b.hashCode())
    a = ParameterSpec.builder("i", INT).addModifiers(KModifier.NOINLINE).build()
    b = ParameterSpec.builder("i", INT).addModifiers(KModifier.NOINLINE).build()
    assertThat(a == b).isTrue()
    assertThat(a.hashCode()).isEqualTo(b.hashCode())
  }
  ...
  fun escapeKeywordInParameterName() {
    val parameterSpec = ParameterSpec.builder("if", STRING).build()
    assertThat(parameterSpec.toString()).isEqualTo("`if`: kotlin.String")
  }

  @Test
  fun escapePunctuationInParameterName() {
    val parameterSpec = ParameterSpec.builder("with-hyphen", STRING).build()
  ...
  fun generalBuilderEqualityTest() {
    val parameterSpec =
      ParameterSpec.builder("Nuts", STRING)
        .addAnnotation(ClassName("io.github.kavikt", "Food"))
  ...
  @Test
  fun modifyModifiers() {
    val builder = ParameterSpec.builder("word", STRING).addModifiers(KModifier.NOINLINE)
  ...
  fun modifyAnnotations() {
    ...
    val builder =
      ParameterSpec.builder("word", STRING)
  ...
  fun doublePropertyInitialization() {
    val codeBlockDefaultValue =
      ParameterSpec.builder("listA", STRING)
    ...
    val formatDefaultValue =
      ParameterSpec.builder("listA", STRING).defaultValue("foo").defaultValue("bar").build()

    assertThat(CodeBlock.of("bar")).isEqualTo(formatDefaultValue.defaultValue)
  }

  // kmp: this test was extracted: jvmModifiersAreNotAllowed
