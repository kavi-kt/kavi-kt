...
/**
 * Make a TypeVariableName for the given TypeMirror. This form is used internally to avoid infinite
 * recursion in cases like `Enum<E extends Enum<E>>`. When we encounter such a thing, we will make a
 * TypeVariableName without bounds and add that to the `typeVariables` map before looking up the
 * bounds. Then if we encounter this TypeVariable again while constructing the bounds, we can just
 * return it from the map. And, the code that put the entry in `variables` will make sure that the
...
private val JAVA_OBJECT = TypeVariableName.JAVA_OBJECT
...
public operator fun TypeVariableName.Companion.invoke(
  ...
  TypeVariableName.of(
    name = name,
    bounds = bounds.map(Type::asTypeName).ifEmpty(TypeVariableName.Companion::NULLABLE_ANY_LIST),
    variance = variance,
  )
...
public operator fun TypeVariableName.Companion.invoke(
  ...
  TypeVariableName.of(
    name,
    bounds.map { it.asTypeName() }.ifEmpty(TypeVariableName.Companion::NULLABLE_ANY_LIST),
    variance,
  )
...
@JvmName("get")
public fun javax.lang.model.type.TypeVariable.asTypeVariableName(): TypeVariableName =
...
public fun TypeParameterElement.asTypeVariableName(): TypeVariableName {
  ...
  val boundsTypeNames =
    bounds.map(TypeMirror::asTypeName).ifEmpty(TypeVariableName.Companion::NULLABLE_ANY_LIST)
