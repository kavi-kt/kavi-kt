  ...
  return mirror.accept(
    object : SimpleTypeVisitor8<TypeName, Void?>() {
      ...
      override fun visitDeclared(t: DeclaredType, p: Void?): TypeName {
        ...
        val enclosing =
          if (enclosingType.kind != TypeKind.NONE && Modifier.STATIC !in t.asElement().modifiers) {
...
internal fun TypeName.Companion.get(type: Type, map: MutableMap<Type, TypeVariableName>): TypeName {
  return when (type) {
    is Class<*> ->
      when {
        type === Void.TYPE -> UNIT
