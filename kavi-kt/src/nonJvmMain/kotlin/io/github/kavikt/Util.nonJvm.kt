...
/**
 * Mirrors `java.lang.Character.isJavaIdentifierStart` for BMP code points: a letter, a letter
 * number (Nl), a currency symbol (Sc, e.g. `$`), or a connecting punctuation character (Pc, e.g.
 * `_`). Supplementary letters are unsupported off the JVM (see CodePoint).
 */
internal actual fun Char.isJavaIdentifierStart(): Boolean {
  return isLetter() ||
    this in CharCategory.LETTER_NUMBER ||
    this in CharCategory.CURRENCY_SYMBOL ||
    this in CharCategory.CONNECTOR_PUNCTUATION
}

/**
 * Mirrors `java.lang.Character.isJavaIdentifierPart` for BMP code points: everything accepted by
 * [isJavaIdentifierStart] plus digits (Nd), combining marks (Mc/Mn), and ignorable characters.
 */
internal actual fun Char.isJavaIdentifierPart(): Boolean {
  return isLetter() ||
    ...
    this in CharCategory.CURRENCY_SYMBOL ||
    this in CharCategory.CONNECTOR_PUNCTUATION ||
    this in CharCategory.COMBINING_SPACING_MARK ||
